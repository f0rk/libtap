...
B<Signature:> int tap_like(char *val, char *regex, char *message, int flags)
...
found in C<regex.h>. See also C<man regex>.

=head2 tap_unlike

B<Signature:> int tap_unlike(char *val, char *regex, char *message, int flags)
